...
receiptsRouter.get('/', userExtractor, async (req: Request, res: Response) => {
  ...
  const querySpec = {
    ...
    parameters: [{ name: '@userEAddress', value: user.eAddressId }],
  }
...
receiptsRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const querySpec = {
      query:
        'SELECT * FROM root r WHERE r.eAddressId = @userEAddress AND (IS_DEFINED(r.forwardedUsers) AND NOT IS_NULL(r.forwardedUsers))',
      parameters: [{ name: '@userEAddress', value: user.eAddressId }],
    }
    const values = await findReceipts(querySpec)
...
receiptsRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const querySpec = {
      query:
        'SELECT r.eAddressId FROM root r WHERE (IS_DEFINED(r.forwardedUsers) AND ARRAY_CONTAINS(r.forwardedUsers, @requesterId))',
      parameters: [{ name: '@requesterId', value: user.id }],
    }
    ...
    const b = values.reduce(
      (val, o) => ((val[o.eAddressId] = (val[o.eAddressId] || 0) + 1), val),
      {}
    )
    ...
    for (let a in b) {
      const querySpec = {
        ...
        parameters: [{ name: '@eAddress', value: a }],
      }
...
receiptsRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (user.eAddressId === req.params.eAddressId) {
      return res
        .status(400)
        .json({ error: 'user cannot have receipts forwarded from themself' })
    }
    const querySpec = {
      query:
        'SELECT * FROM root r WHERE r.eAddressId = @requestedEAddress AND (IS_DEFINED(r.forwardedUsers) AND ARRAY_CONTAINS(r.forwardedUsers, @requesterId))',
      parameters: [
...
receiptsRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!item) {
      return res
        .status(500)
        .json({ error: 'database: receipt matching given id not found' })
    }
    ...
    if (item.eAddressId === user.eAddressId) {
      ...
      return res.status(400).json({
        error:
          'access denied: current user is not the owner of this receipt and it is not forwarded to anyone',
      })
    }
  }
)
...
receiptsRouter.post('/', userExtractor, async (req: Request, res: Response) => {
  ...
  if (item.eAddressId !== user.eAddressId) {
    return res
      .status(400)
      .json({ error: 'eAddress is not valid for current user' })
  }
...
receiptsRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (recipientAddress === user.eAddressId) {
      return res
        .status(400)
        .json({ error: 'forwarding receipts to oneself is not allowed' })
    }
    ...
    if (!receipt) {
      return res
        .status(500)
        .json({ error: 'database: no receipt with given id was found' })
    } else if (receipt.eAddressId !== user.eAddressId) {
      return res.status(400).json({
        error:
          'forwarding receipts is only allowed for the original owner of the receipt',
      })
    }

    const querySpec = {
      query: 'SELECT * FROM users u WHERE u.eAddressId = @eAddress',
      parameters: [{ name: '@eAddress', value: recipientAddress }],
    }
    ...
    if (!userItem) {
      return res
        .status(500)
        .json({ error: 'database: no user with given eAddress was found' })
    }
    ...
    if (receipt.forwardedUsers) {
      if (receipt.forwardedUsers.includes(userItem.eAddressId)) {
        return res
          .status(500)
          .json({ error: 'database: user already has access to receipt' })
      }
    }
